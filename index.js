/* =============================================================================
 * EDGE SEO WORKER — www.eryc.my.id
 * Rewrite goals (vs. previous version):
 *   1. FLOOR + CEILING CSS:
 *        - FLOOR  = hand-authored critical CSS shipped IN this worker bundle.
 *                   Never cold, never depends on KV/cache. Guarantees a correct
 *                   above-fold paint even on a fully cold request.
 *        - CEILING = coverage-generated critical CSS from KV (written by the AGP
 *                    scanner). Real "used" rules, inlined, ~few KB. Overrides/
 *                    enhances the floor when present. If KV is empty, you lose a
 *                    little polish, NOT the whole paint. This kills the 77<->94 cliff.
 *   2. ASYNC GSTATIC: stop synchronously fetching + inlining the ~195KB Google
 *        Sites stylesheet into the HTML doc. That fat, render-blocking document
 *        was why mobile FCP/LCP/SI all pinned at 4.1s on Slow-4G. Now it loads
 *        async (media=print -> onload=all). The inlined coverage ceiling paints
 *        above-fold; gstatic only fills the below-fold long tail.
 *   3. EDGE CACHE (caches.default): smoothing layer so repeat hits skip compute.
 *        NOTE: per-colo, not global, and cannot be pre-warmed everywhere. That's
 *        exactly why floor+async-gstatic (which make the COLD path fast) are the
 *        load-bearing fix and cache is only a bonus.
 *   4. WORKING CRON: warms this colo's cache for key pages.
 *   5. Query-string-safe canonical redirect.
 *
 * NOTE on the FLOOR: the value below is a solid structural safety net. For a
 * stronger floor, run the AGP scanner once, grab the coverage CSS it writes to
 * KV, and paste it into CRITICAL_FLOOR_CSS so your last-known-good critical CSS
 * is compiled into the worker and can never go cold.
 * ============================================================================= */

const CANONICAL_HOST = "www.eryc.my.id";
const DOMAIN = "https://www.eryc.my.id";

// --- FLOOR: always-present critical CSS (ships in the bundle) ---
...
.EmVfjc{display:none!important;opacity:0!important;pointer-events:none!important}
#lcp-heavy-bg{min-height:100vh;background-size:cover;background-position:center;background-repeat:no-repeat}
#lcp-heavy-anim{width:auto;object-fit:contain}
img[width="120"][height="120"]{width:120px;height:120px;object-fit:contain}
`.trim();

// --- THE EXECUTIONER CLASS (signal pruning for AI/social bots) ---
class ElementSlasher {
  element(element) {
    if (element.tagName === 'script') {
      const type = element.getAttribute('type') || '';
      if (type.toLowerCase() === 'application/ld+json') return; // spare schema
    }
    element.remove();
  }
}

// Build a per-variant edge-cache key. Variant prevents serving a bot body to a
// human (and vice-versa). Fragments are ignored by the Cache API, so the variant
// goes in the query string of the SYNTHETIC key URL (never a real fetch).
function buildCacheKey(url, variant) {
  const keyUrl = `https://${CANONICAL_HOST}${url.pathname}?__agpv=${variant}`;
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (isBot) console.log(`[AI-DETECT] ${userAgent} accessed ${url.pathname}`);
    ...
    // --- 0.3 SITEMAP (served before any redirect; identical for all UAs) ---
    ...
    // --- 1. FORCE NAKED -> WWW (query-string safe) & KILL "/home" ---
...
`.trim();
    ...
    if (url.pathname === "/llm.txt") {
      return Response.redirect(`https://${CANONICAL_HOST}/llms.txt`, 301);
    }
    ...
    if (url.pathname.startsWith("/assets/")) {
      ...
      if (object === null) return new Response("Asset not found in R2", { status: 404 });
    ...
    // 6a. Edge-cache fast path. PSI/Lighthouse use a Chrome UA -> human variant,
    //     so caching the human path is what stabilizes the score on repeat hits.
    //     We only cache GETs with no query string and only the human variant
    //     (bot payloads are cheap to recompute and change rarely).
    const variant = isAIBot ? "ai" : isSocialBot ? "soc" : isCrawlerBot ? "crawl" : "hum";
    const cacheable = request.method === "GET" && url.search === "" && variant === "hum";
    const cache = caches.default;
    if (cacheable) {
      const hit = await cache.match(buildCacheKey(url, variant));
      if (hit) return hit;
    }

    // 6b. Fetch origin, bail if not HTML.
    const response = await fetch(request);
    const contentType = response.headers.get("content-type") || "";
    if (!contentType.includes("text/html")) return response;

    // 6c. Fetch AGP ceiling state from KV (defensive, edge-cached read). Floor
    //     covers the empty case, so missing KV is non-fatal.
    let agpLcpUrl = "";
    let agpGhostCss = "";
    try {
    ...
    // 6d. Head payload (meta, OG, JSON-LD). NOTE: gstatic is NOT inlined here
    //     anymore — see the link[rel=stylesheet] handler below.
    const customHeaderContent = `
        ...
        <meta property="og:image" content="https://www.eryc.my.id/assets/image/homepage-screenshot.webp">
        ...
        <meta name="twitter:image" content="https://www.eryc.my.id/assets/image/homepage-screenshot.webp">
        ...
        <script type="application/ld+json">
        {
          ...
          "@graph": [
            ...
            },
            {
              ...
              "image": "https://www.eryc.my.id/assets/image/hero.avif",
            ...
            },
            {
              ...
              "geo": {
                "@type": "GeoCoordinates",
                "latitude": "-7.9839", "longitude": "112.6214",
    ...
    // Helper: inject FLOOR then CEILING into <head>. Cascade order matters —
    // ceiling (coverage CSS) must come AFTER floor so it wins where it exists.
    const injectCriticalCss = (e) => {
    ...
    if (!isBot) {
      ...
      // Edge-cache 10min, browser 60s. Keep modest so deploys propagate fast.
      if (cacheable) newHeaders.set("Cache-Control", "public, s-maxage=600, max-age=60");

      // Ground-truth LCP preload (written by the scanner). Now self-consistent:
      // the scanner measures the post-edge page, so this points at the small poster.
      if (agpLcpUrl) {
...
<script data-edge-ignore="true">
    (function() {
      ...
      const humanRewriter = new HTMLRewriter()
        ...
        .on("head", {
          element(e) {
            ...
            injectCriticalCss(e);          // FLOOR + CEILING
            e.append(wakeUpScript, { html: true });
          }
        })
        .on("div[data-code]", { element(e) { currentEmbedCode = e.getAttribute("data-code"); } })
        .on('img', {
          element(e) {
        ...
        .on('picture > source', { element(e) { e.removeAttribute("srcset"); } })
        .on("iframe.YMEQtf", {
          element(e) {
            if (currentEmbedCode) {
        ...
        .on('div[role="button"][aria-haspopup="true"]', {
          element(e) {
            if (!e.hasAttribute('aria-label')) e.setAttribute('aria-label', 'Open Navigation Menu');
          }
        })
        .on('script', {
          element(e) {
        ...
        // 🚀 GSTATIC: defer async instead of synchronous inline. The coverage
        //    ceiling already paints above-fold; this only fills the below-fold
        //    long tail. This is the single change that unblocks mobile FCP/LCP.
      ...
      const transformed = humanRewriter.transform(response);
      const finalResp = new Response(transformed.body, { status: response.status, headers: newHeaders });

      // Store in edge cache (per-colo). clone() so we can still return the stream.
      if (cacheable && response.status === 200) {
        ctx.waitUntil(cache.put(buildCacheKey(url, variant), finalResp.clone()));
      }
      return finalResp;
    }
    ...
    const rewriter = new HTMLRewriter()
      ...
      .on("head", {
        element(e) {
          e.append(customHeaderContent, { html: true });
          injectCriticalCss(e);            // FLOOR + CEILING for bots too
        }
      });

    if (isBot && botPayload) {
      rewriter.on("body", { element(element) { element.prepend(botPayload, { html: true }); } });
    }

    // Signal pruning for AI/social bots (Googlebot keeps the full page).
    ...
    if (agpLcpUrl) {
      botHeaders.append('Link', `<${agpLcpUrl}>; rel=preload; as=image`);
    }
  ...
  // --- 7. CRON: warm THIS colo's edge cache for key pages ---
  //     (KV refresh for the AGP ceiling lives in the separate scanner worker.)
  async scheduled(event, env, ctx) {
    console.log(`Cron triggered at ${event.scheduledTime}`);
    const pages = ["/", "/about", "/glossary", "/case-studies/edge-seo"];
    ctx.waitUntil(Promise.all(
      pages.map(p => fetch(`https://${CANONICAL_HOST}${p}`, {
        headers: { "User-Agent": "Mozilla/5.0 (cache-warmer)" }
      }).catch(err => console.error("Warm failed:", p, err)))
    ));
  }
};
